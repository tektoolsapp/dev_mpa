...

{
    ...
    public function getMembers($types=null)
    {
        if(!is_null($types)) {
        ...
        $members = $this->leftJoin('member_types', 'members.member_type', '=', 'member_types.member_type_value')
            ...
            ->when($type_c, function ($q) use ($type_c) {
                return $q->orwhere('members.member_status', $type_c);
            })
            ->when($type_p, function ($q) use ($type_p) {
                return $q->orwhere('members.member_status', $type_p);
            })
            ->when($type_u, function ($q) use ($type_u) {
                return $q->orwhere('members.member_status', $type_u);
            })
            ->select(
            'members.id',
